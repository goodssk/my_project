...
### 完全是numpy和pandas的用法，感觉numpy和pandas能玩到炉火纯青的地步，要打到这个地步就行了， 牛逼
f = open('../data/zeng.txt', 'r', encoding='utf-8')  # 读取文章
s = f.read()  # 读取为一个字符串
...
myre = {2: '([\u4E00-\u9FFF]{2})', 3: '([\u4E00-\u9FFF]{3})', 4: '([\u4E00-\u9FFF]{4})', 5: '([\u4E00-\u9FFF]{5})', 6: '([\u4E00-\u9FFF]{6})', 7: '([\u4E00-\u9FFF]{7})'}

min_count = 10  # 录取词语最小出现次数
min_support = 5  # 录取词语最低支持度，1代表着随机组合
...
for m in range(2, max_sep + 1):
    ...
    for k in range(m - 1):
        qq = np.array(list(map(lambda ms: tsum * t[m - 1][ms] / t[m - 2 - k][ms[:m - 1 - k]] / t[k][ms[m - 1 - k:]],
                               tt.index))) > min_support  # 最小支持度筛选, 这个计算的应该是凝聚度
        tt = tt[qq]
    rt.append(tt.index)
    print('rt', rt)
